module.exports = {
	...
	async execute(message, args) {
		...
		const countingChannel = await message.client.channels.fetch(
			guildSettings.countingChannelID,
		);
		...
		if (desiredState === undefined) {
			return message.channel.send(`Failed to find message with number ${args[0]}!`);
		}
		...
		while (curMsg !== desiredState) {
			curMsg.delete(1000).catch(`Failed to delete message '${curMsg.content}'!`);
